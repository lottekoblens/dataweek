...
const PieChart = ({ data }) => {
	...
	const ref = useD3(svg => {
		...
		arcs.data(pie.value(d => d.nummer)(data)) // add data to pie chart
			.enter()
			.append('path')
			.attr('class', 'arc')
		...
		// arcs.data(pie.value(d => d.number)(data))
		// 	.enter()
		// 	.append('text') // add text to arcs
		// 	.text(function (d) {
		// 		const scores = data.map(d => d.number) // scores are all values of listeners that the pie chart consists off
		// 		const totalScores = scores.reduce(
		// 			(previousScore, currentScore) => previousScore + currentScore, 0 // calculate total
		// 		)
		// 		return Math.round((d.data.number / totalScores) * 100) + '%' // calculate the percentage of the arc of the total
		// 	})
		...
		// 		.text(
		// 			"Het nummer " + data.data.name +
		// 			" heeft " + data.data.number + ' luisteraars' // give tooltip this text with the right data
		// 		)
		// }
